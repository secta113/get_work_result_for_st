# --- network_handler.py (V6.2) ---
# (新規作成)
# 役割: requestsによる通信、HTMLのパースを担当する

import logging
import os # ★ V6.2: os.getenv のためインポート
import re 
...
# ★ V6.2: .env からカンパニーコードを読み込む
# (app.py で load_dotenv 済みなので getenv が使えます)
...
if COMPANY_CODE == "witesi" and not os.getenv("LOGIN_COMPANY_CODE"):
    logger.warning("LOGIN_COMPANY_CODE が .env に設定されていません。デフォルト値 'witesi' を使用します。")
     ...
     logger.error("LOGIN_COMPANY_CODE が .env に設定されていますが、値が空です。")
     COMPANY_CODE = "witesi" # ★ V6.2: 空の場合はフォールバック

LOGIN_PAGE_URL = f"{BASE_URL}/Login.aspx?c={COMPANY_CODE}" # ★ V6.2: 変数を使用
