...
# V5.0 ベース
# V6.0 変更点:
# 1. 処理ロジックを main_controller.py に分離
# 2. 日付生成ロジックを date_utils.py に分離
# 3. CSV/Network/集計ロジック (旧 automation_logic) は main_controller が呼び出す
# 4. app.py は UI と main_controller の呼び出し/結果表示に専念
...
if getattr(sys, 'frozen', False):
    ...
    # (Python実行時 / bat実行時)
    # APP_BUNDLE_DIR = os.path.dirname(os.path.abspath(__file__)) # (変更前) [cite: 8]
    # ★ 変更: apprun.bat で cd core されるため、CWD (core) を基準
    APP_BUNDLE_DIR = os.path.abspath(os.getcwd())
    ROOT_DIR = os.path.abspath(os.path.join(APP_BUNDLE_DIR, ".."))

# V6.1: .env は app.py と同じ場所 (core内)
env_path = os.path.join(APP_BUNDLE_DIR, ".env") # [cite: 8]
load_dotenv(env_path) # [cite: 8]
...
# ===============================================
# ▼▼▼ Streamlit の UI と メインロジック (V6.0) ▼▼▼
# ===============================================

# --- 1. UIの定義 ---
st.set_page_config(page_title="給与明細 自動取得", layout="centered")
st.title("給与明細 自動取得ツール 🤖 (V6.1)") # ★ バージョン更新
st.write("（V6.1: 処理ロジックをモジュール分割 / bat対応）")
logging.info("Streamlit UI ページがロードされました。 (V6.1)")

initial_id = os.getenv("MY_LOGIN_ID", "")
initial_pw = os.getenv("MY_PASSWORD", "")
